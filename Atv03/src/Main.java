...
void main() {
    ...
    for (int i = 0; i < 3; i++){
        ...
        switch (tipoAluno) {
            ...
            case "bolsista":
                ...
                alunos[i] = bolsista;
                bolsista.setMensalidade(bolsista.calcularMensalidadeBolsista());
                break;

            case "integral":
                Integral integral = new Integral(matricula,nome, email, nascimento, serie);
                alunos[i] = integral;
                integral.setMensalidade(integral.calcularMensalidadeIntegral());
                break;
        }
    }
    ...
    for (int i = 0; i < 3; i++){
        ...
        System.out.println("Valor da mensalidade: R$" + alunos[i].getMensalidade());
        System.out.println("----------------------------------------------");
    }
